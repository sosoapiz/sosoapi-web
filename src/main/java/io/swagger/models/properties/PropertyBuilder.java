 ...
 * 
		...
		* <p>CreateDate: 2015年9月8日上午11:37:53</p>
 */
public class PropertyBuilder {
    ...
    private enum Processor {
        ...
        REFERENCE(RefProperty.class) {
            ...
            public Property merge(Property property, Map<PropertyId, Object> args) {
                ...
                if (property instanceof RefProperty) {
                    ...
                    if (args.containsKey(PropertyId.REF)) {
                        final String value = PropertyId.REF.findValue(args);
                        if (value != null) {
        ...
        JSON(JsonProperty.class) {
            @Override
            protected boolean isType(String type, String format) {
                return JsonProperty.isType(type, format);
            }

            @Override
            protected JsonProperty create() {
            ...
            public Property merge(Property property, Map<PropertyId, Object> args) {
                ...
                if (property instanceof JsonProperty) {
                    final JsonProperty resolved = (JsonProperty) property;
                    if (args.containsKey(PropertyId.EXT_SCHEMA)) {
                        final String value = PropertyId.EXT_SCHEMA.findValue(args);
                        if (value != null) {
                            resolved.setContent(value);
                        }
                    }
                }
                return property;
            ...
            public Model toModel(Property property) {
                if (property instanceof JsonProperty) {
                    final JsonProperty resolved = (JsonProperty) property;
                    final CustModel model = new CustModel();
                    model.setType(resolved.getType());
                    model.setFormat(resolved.getFormat());
                    model.setContent(resolved.getContent());
                    model.setName(resolved.getName());
                    model.setDescription(resolved.getDescription());
                    return model;
                }
                return null;
